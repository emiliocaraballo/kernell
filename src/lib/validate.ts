...
const validateForm = (type: TypeValidateStrings, str: any, lengthmin = 0, lengthmax = 0): ResponseValidateForm => {
    ...
    if (!(str.length >= lengthmin) && lengthmin > 0 && res) {
        res = false;
        code=2;
    }
    if (!(str.length <= lengthmax) && lengthmax > 0 && res) {
        res = false;
        code=3;
    }
    /* 
        ...
        code:1
         ...
         // no cumple con la condicion minima de caracteres
        status=false
        code:2

        // no cumple con la condicion maxima de caracteres
        status=false
        code:3
    */
    return {
